 ...
 *
 * <p>ECKey also provides access to Digitalcoin-Qt compatible text message signing, as accessible via the UI or JSON-RPC.
...
public class ECKey implements Serializable {
    ...
    static {
        // All clients must agree on the curve to use by agreement. Digitalcoin uses secp256k1.
    ...
    /**
     * Construct an ECKey from an ASN.1 encoded private key. These are produced by OpenSSL and stored by the Digitalcoin
    ...
    /**
     * Output this ECKey as an ASN.1 encoded private key, as understood by OpenSSL or used by the Digitalcoin reference
     ...
     * how ECDSA signatures are represented when embedded in other data structures in the Digitalcoin protocol. The raw
    ...
    public static class ECDSASignature {
         ...
         * of the type used by Digitalcoin we have to encode them using DER encoding, which is just a way to pack the two
    ...
    /**
     * Signs the given hash and returns the R and S components as BigIntegers. In the Digitalcoin protocol, they are
    ...
    /**
     * Signs a text message using the standard Digitalcoin messaging signing format and returns the signature as a base64
     * encoded string.
     *
    ...
    /**
     * Given an arbitrary piece of text and a Digitalcoin-format message signature encoded in base64, returns an ECKey
     * containing the public key that was used to sign it. This can then be compared to the expected public key to
     * determine if the signature was correct. These sorts of signatures are compatible with the Digitalcoin-Qt/digitalcoind
     * format generated by signmessage/verifymessage RPCs and GUI menu options. They are intended for humans to verify
     * their communications with each other, hence the base64 format and the fact that the input is text.
     *
     * @param message Some piece of human readable text.
     * @param signatureBase64 The Digitalcoin-format message signature in base64
     ...
     *
     * Because it is a critical failure if the private keys cannot be decrypted successfully (resulting of loss of all digitalcoins controlled
