...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Digitalcoin proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param protocolVersion Digitalcoin protocol version.
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = digitalcoinSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Digitalcoin protocol.
    ...
    /**
     * Should only used by DigitalcoinSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by DigitalcoinSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeDigitalcoinSerialize()}, which is safe to mutate.
    ...
    public byte[] digitalcoinSerialize() {
        byte[] bytes = unsafeDigitalcoinSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the digitalcoin wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeDigitalcoinSerialize() {
        ...
        try {
            digitalcoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to digitalcoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the digitalcoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void digitalcoinSerialize(OutputStream stream) throws IOException {
    ...
    void digitalcoinSerializeToStream(OutputStream stream) throws IOException {
        log.debug("Warning: {} class has not implemented digitalcoinSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so DigitalcoinSerializer can avoid 2 instanceof checks + a casting.
     *
     * @return
     */
