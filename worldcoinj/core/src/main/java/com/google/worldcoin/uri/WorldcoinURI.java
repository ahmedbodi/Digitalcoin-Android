...
/**
 * <p>Provides a standard implementation of a digitalcoin URI with support for the
 * following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code digitalcoin:<address>}</li>
 * <li>{@code digitalcoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://en.digitalcoin.it/wiki/BIP_0021">BIP 0021</a>
 */
public class DigitalcoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(DigitalcoinURI.class);
    ...
    public static final String LITECOIN_SCHEME = "digitalcoin";
    ...
    /**
     * Constructs a new DigitalcoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws DigitalcoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public DigitalcoinURI(String uri) {
        this(null, uri);
    }
     ...
     * @throws DigitalcoinURIParseException If the input fails Digitalcoin URI syntax and semantic checks.
     */
    public DigitalcoinURI(NetworkParameters params, String input) {
        // Basic validation
        ...
        try {
            ...
            throw new DigitalcoinURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  digitalcoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form digitalcoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the digitalcoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : digitalcoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("digitalcoin://")) {
            schemeSpecificPart = input.substring("digitalcoin://".length());
        } else if (input.startsWith("digitalcoin:")) {
            schemeSpecificPart = input.substring("digitalcoin:".length());
        } else {
            throw new DigitalcoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0 || "".equals(addressSplitTokens[0])) {
            throw new DigitalcoinURIParseException("Missing address");
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new DigitalcoinURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
    ...
    private void parseParameters(NetworkParameters params, String addressToken, String[] nameValuePairTokens) {
        // Attempt to parse the addressToken as a Digitalcoin address for this network
        try {
            Address address = new Address(params, addressToken);
            putWithValidation(FIELD_ADDRESS, address);
        } catch (final AddressFormatException e) {
            throw new DigitalcoinURIParseException("Bad address", e);
        }
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (tokens.length != 2 || "".equals(tokens[0])) {
                throw new DigitalcoinURIParseException("Malformed Digitalcoin URI - cannot parse name value pair '" +
                        nameValuePairToken + "'");
            }
    ...
    private void putWithValidation(String key, Object value) {
        if (parameterMap.containsKey(key)) {
            throw new DigitalcoinURIParseException("'" + key + "' is duplicated, URI is invalid");
        } else {
    ...
    /**
     * @return The Digitalcoin Address from the URI
     */
     ...
     *         10,000,000 units is 1 WDC. May be null if no amount is specified
     */
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("DigitalcoinURI[");
        boolean first = true;
    ...
    public static String convertToDigitalcoinURI(Address address, BigInteger amount, String label, String message) {
        return convertToDigitalcoinURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Digitalcoin URI builder using known good fields.
     * 
     * @param address The Digitalcoin address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the Digitalcoin URI
     */
    public static String convertToDigitalcoinURI(String address, BigInteger amount, String label, String message) {
        ...
        if (amount != null) {
            ...
            builder.append(Utils.digitalcoinValueToPlainString(amount));
