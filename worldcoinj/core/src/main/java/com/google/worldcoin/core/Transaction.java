 ...
 * the minting of new coins. A Transaction object corresponds to the equivalent in the Digitalcoin C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of Digitalcoin and have many powerful features. Read
 * <a href="http://code.google.com/p/feathercoinj/wiki/WorkingWithTransactions">"Working with transactions"</a> in the
 * documentation to learn more about how to use this class.</p>
 *
 * <p>All Digitalcoin transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage implements Serializable {
    ...
    // These are serialized in both digitalcoin and java serialization.
     ...
     * @param msg Digitalcoin protocol formatted byte array containing message content.
    ...
    public Sha256Hash getHash() {
        if (hash == null) {
            byte[] bits = digitalcoinSerialize();
    ...
    /**
     * Used by DigitalcoinSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of Digitalcoin share. In 2011 the value of a coinbase
    ...
    public String toString(AbstractBlockChain chain) {
        ...
        for (TransactionOutput out : outputs) {
            ...
            try {
                ...
                s.append(" ");
                s.append(digitalcoinValueToFriendlyString(out.getValue()));
                s.append(" WDC");
     ...
     * features of Digitalcoin are developed.
     *
    ...
    synchronized Sha256Hash hashTransactionForSignature(int inputIndex, byte[] connectedScript,
        ...
        //
        //   https://en.digitalcoin.it/wiki/Contracts
        ...
        try {
            ...
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in Feathercoin 0.1.
            ...
            if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
                ...
                if (inputIndex >= this.outputs.size()) {
                    ...
                    // Digitalcoin implementation. The reference client also contains a bug in handling this case:
    ...
    protected void digitalcoinSerializeToStream(OutputStream stream) throws IOException {
        ...
        for (TransactionInput in : inputs)
            in.digitalcoinSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.digitalcoinSerialize(stream);
     ...
     * This is useful in certain types of <a href="http://en.digitalcoin.it/wiki/Contracts">contracts</a>, such as
