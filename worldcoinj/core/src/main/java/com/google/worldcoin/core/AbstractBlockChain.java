 ...
 * the lightweight SPV mode, a {@link com.google.digitalcoin.store.BoundedOverheadBlockStore} may be a good choice.</p>
 *
...
public abstract class AbstractBlockChain {
     ...
     *
     * Following this one down to the genesis block produces the story of the economy from the creation of Digitalcoin
    ...
    private boolean add(Block block, Set<Sha256Hash> filteredTxHashList, List<Transaction> filteredTxn, boolean tryConnecting)
        ...
        try {
            ...
            // article here for more details: http://code.google.com/p/digitalcoinj/wiki/SecurityModel
            try {
                block.verifyHeader();
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener.isTransactionRelevant(tx)) {
                    if (clone)
                        tx = new Transaction(tx.params, tx.digitalcoinSerialize());
    ...
    private void checkDifficultyTransitions(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        ...
        int interval = nTargetTimespanCurrent/params.TARGET_SPACING;
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // There is an integer underflow bug in digitalcoin-qt that means mindiff blocks are accepted when time
        // goes backwards.
