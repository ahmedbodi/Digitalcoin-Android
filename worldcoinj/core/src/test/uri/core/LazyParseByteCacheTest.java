...
public class LazyParseByteCacheTest {
    ...
    public void setUp() throws Exception {
        ...
        DigitalcoinSerializer bs = new DigitalcoinSerializer(unitTestParams);
        
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        bs.serialize(tx1, bos);
        tx1BytesWithHeader = bos.toByteArray();
        tx1Bytes = tx1.digitalcoinSerialize();
        
        bos.reset();
        bs.serialize(tx2, bos);
        tx2BytesWithHeader = bos.toByteArray();
        tx2Bytes = tx2.digitalcoinSerialize();
        
        bos.reset();
        bs.serialize(b1, bos);
        b1BytesWithHeader = bos.toByteArray();
        b1Bytes = b1.digitalcoinSerialize();
    }
    ...
    public void testBlock(byte[] blockBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
    	...
    	//message structure.
    	DigitalcoinSerializer bsRef = new DigitalcoinSerializer(unitTestParams, false, false);
    	ByteArrayOutputStream bos = new ByteArrayOutputStream();
    	
    	DigitalcoinSerializer bs = new DigitalcoinSerializer(unitTestParams, lazy, retain);
    	Block b1;
    	Block bRef;
    	...
    	//verify our reference DigitalcoinSerializer produces matching byte array.
    	bos.reset();
    ...
    public void testTransaction(NetworkParameters params, byte[] txBytes, boolean isChild, boolean lazy, boolean retain) throws Exception {
    	...
    	//reference serializer to produce comparison serialization output after changes to
    	//message structure.
    	DigitalcoinSerializer bsRef = new DigitalcoinSerializer(params, false, false);
    	ByteArrayOutputStream bos = new ByteArrayOutputStream();
    	
    	DigitalcoinSerializer bs = new DigitalcoinSerializer(params, lazy, retain);
    	Transaction t1;
    	Transaction tRef;
    ...
    private void serDeser(DigitalcoinSerializer bs, Message message, byte[] sourceBytes, byte[] containedBytes, byte[] containingBytes) throws Exception {
    	ByteArrayOutputStream bos = new ByteArrayOutputStream();
