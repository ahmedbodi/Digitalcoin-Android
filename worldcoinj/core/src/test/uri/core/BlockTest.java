...
public class BlockTest {
    ...
    public void testHeaderParse() throws Exception {
        ...
        Block reparsed = new Block(params, header.digitalcoinSerialize());
    ...
    public void testBitCoinSerialization() throws Exception {
        ...
        // of transactions.
        Block block = new Block(params, blockBytes);
        assertTrue(Arrays.equals(blockBytes, block.digitalcoinSerialize()));
    }
    ...
    public void testJavaSerialiazation() throws Exception {
        ...
        // Note that this will actually check the transactions are equal by doing digitalcoin serialization and checking
    ...
    public void testUpdateLength() {
        ...
        assertEquals(block.digitalcoinSerialize().length, block.length);
        final int origBlockLen = block.length;
        Transaction tx = new Transaction(params);
        // this is broken until the transaction has > 1 input + output (which is required anyway...)
        //assertTrue(tx.length == tx.digitalcoinSerialize().length && tx.length == 8);
        ...
        assertEquals(tx.digitalcoinSerialize().length, tx.length);
        ...
        assertEquals(block.length, block.digitalcoinSerialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
