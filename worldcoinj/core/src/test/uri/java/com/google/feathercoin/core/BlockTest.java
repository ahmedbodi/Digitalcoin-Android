...
public class BlockTest {
    ...
    public void testHeaderParse() throws Exception {
        ...
        Block reparsed = new Block(params, header.feathercoinSerialize());
        assertEquals(reparsed, header);
    }

    @Test
    public void testFeathercoinSerialization() throws Exception {
        ...
        // of transactions.
        Block block = new Block(params, blockBytes);
        assertTrue(Arrays.equals(blockBytes, block.feathercoinSerialize()));
    }
    ...
    public void testJavaSerialiazation() throws Exception {
        ...
        // Note that this will actually check the transactions are equal by doing feathercoin serialization and checking
    ...
    public void testUpdateLength() {
        ...
        assertEquals(block.feathercoinSerialize().length, block.length);
        final int origBlockLen = block.length;
        Transaction tx = new Transaction(params);
        // this is broken until the transaction has > 1 input + output (which is required anyway...)
        //assertTrue(tx.length == tx.feathercoinSerialize().length && tx.length == 8);
        ...
        assertEquals(tx.feathercoinSerialize().length, tx.length);
        ...
        assertEquals(block.length, block.feathercoinSerialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
